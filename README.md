...
## 🎸 기타
### **🌸 느낀 점 및 후기**
- **이연규 (팀장)**: 프로젝트 초기에 계획을 잘 세우고 지키는 것이 중요하다는 것을 느꼈다. 그런 부분에서 구체적인 계획을 세우고 업무를 분담시킬 수 있는 능력이 있는 팀원이 있다는 것이 큰 도움이 됐다. 협업하는 과정에서 실수할 것을 두려워 하지 않고 부딪혀 나가는 것에서 배운 점이 많은 것 같다. 시간이 부족해서 완벽한 구현은 안됐지만 초기에 생각했던 것 보다 더 근사한 완성품이 나와서 개인적으로는 만족스러운 프로젝트였다.
- **신예주**: 이번에 참여한 첫 개발 프로젝트를 통해 개개인의 능력뿐만 아니라 팀원들 간의 소통, 협력 또한 중요하다는 것을 더욱 깨닫게 되었다. 또한 팀원 모두 백/프론트를 담당했던 경험은 추후에도 큰 도움이 될 것이라 생각한다. 앞으로의 팀 프로젝트에서도 체계적인 프로세스를 가지고 최선을 다해 임할 것이다 !
