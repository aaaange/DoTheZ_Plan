...
## 🎸 기타
### **🌸 느낀 점 및 후기**
- **이연규 (팀장)**: 프로젝트 초기에 계획을 잘 세우고 지키는 것이 중요하다는 것을 느꼈다. 그런 부분에서 구체적인 계획을 세우고 업무를 분담시킬 수 있는 능력이 있는 팀원이 있다는 것이 큰 도움이 됐다. 협업하는 과정에서 실수할 것을 두려워 하지 않고 부딪혀 나가는 것에서 배운 점이 많은 것 같다. 생각보다 시간이 부족해서 완벽한 구현은 안됐지만 초기에 생각했던 것 보다 더 근사한 완성품이 나와서 개인적으로는 만족스러운 프로젝트였다.
- **신예주**: 
